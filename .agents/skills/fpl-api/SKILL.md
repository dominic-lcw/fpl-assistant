...
| Private form beliefs | `src/lib/fpl/beliefs.ts` + `player_beliefs` | `upsert_player_belief`, `list_player_beliefs`, `get_player_belief`, `clear_player_belief` | Per-user form priors that adjust `recommendationScore` in suggestions/squad builds |
...
11. Form beliefs are **per signed-in user only** — never share or leak across users. Upsert with evidence before construction when form diverges from the API.
