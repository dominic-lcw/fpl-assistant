...
| Private player beliefs | `src/lib/fpl/beliefs.ts` + `theses.ts` + `form_theses` / `player_beliefs` | `compute_player_expectation`, `upsert_player_belief`, … | Beliefs are primary; thesis is an optional tag/bag. Upsert auto-creates a tag. Squad build does not require synthesis. |
...
11. Player beliefs are **per signed-in user only**. Workflow: `compute_player_expectation` (quantify) → `upsert_player_belief` (auto-creates a thesis tag) → `suggest_squad`. `create_form_thesis` / `synthesize_form_thesis` are optional labels/notes only. Never share across users. `expectedPoints` comes from the calculation tool / upsert auto-fill — do not invent it.
