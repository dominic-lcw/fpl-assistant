...
function ContextDisplayRootBase({
  ...
  // Keep the last non-zero usage so the ring does not flicker to 0 while a
  // new reply is streaming (before metadata arrives).
  ...
  if (tokenState.threadId !== threadId) {
    setTokenState({
      threadId,
      totalTokens: rawTokens > 0 ? rawTokens : 0,
      usage,
    ...
    setTokenState({
      threadId,
      totalTokens: rawTokens > 0 ? rawTokens : tokenState.totalTokens,
      usage: rawTokens > 0 ? usage : tokenState.usage,
    });
  }
    ...
    >
      <div className="text-xs">
        <div className="flex items-baseline justify-between gap-6 whitespace-nowrap">
          <span className="font-medium">Last reply usage</span>
...
const ContextDisplayRing: FC<PresetProps> = ({
  ...
  <ContextDisplayRoot modelContextWindow={modelContextWindow} usage={usage}>
    <ContextDisplayTrigger
      ...
      aria-label="Last reply token usage"
    >
      <RingVisual />
      <RingPercentLabel />
    </ContextDisplayTrigger>
